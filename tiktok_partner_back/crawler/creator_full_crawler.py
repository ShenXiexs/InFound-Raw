...
class CreatorFullCrawler:
    ...
    def navigate_to_creator_connection(self, page) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning("未找到欢迎文字，尝试重新开始登录")
                return False
    ...
    def parse_search_strategy(self):
        ...
        # 解析产品类别
        product_cat = strategy.get("product_category", "")
        if isinstance(product_cat, str):
            self.product_categories = [cat.strip() for cat in product_cat.split(",") if cat.strip()]
        elif isinstance(product_cat, list):
            self.product_categories = [str(c).strip() for c in product_cat if str(c).strip()]
        else:
        ...
        if isinstance(gender_info, str):
            ...
            if not s:
                ...
                m = re.search(r'(\d+)%', s)
                pct = int(m.group(1)) if m else 70
        ...
        # 解析GMV范围（支持多地区货币格式）
        gmv = strategy.get("min_GMV", 100)

        # 根据地区确定GMV范围选项
        if self.region == 'FR':
            all_gmv_ranges = ["0 €-100 €", "100 €-1.000 €", "1.000 €-10K €", "10K €+"]
        else:  # 默认MX或其他地区
            all_gmv_ranges = ["$0-$100", "$100-$1K", "$1K-$10K", "$10K+"]

        gmv_thresholds = [0, 100, 1000, 10000]

        if isinstance(gmv, (int, float, str)):
            try:
                # 清理货币符号
                gmv_str = str(gmv).replace("$", "").replace("€", "").replace("£", "")
                gmv_str = gmv_str.replace(",", "").strip()
                gmv_val = float(gmv_str) if gmv_str else 0
                
                self.gmv_ranges = []
                
                # 根据最小值选择符合的范围
                for i, threshold in enumerate(gmv_thresholds):
                    if gmv_val <= threshold:
                        self.gmv_ranges = all_gmv_ranges[i:]
                        break
                
                # 如果最小值超过所有阈值
                if not self.gmv_ranges and gmv_val > gmv_thresholds[-1]:
                    self.gmv_ranges = []  # 没有符合的范围
                        
            except:
                # 如果解析失败，使用默认
                self.gmv_ranges = all_gmv_ranges[1:]
                
        elif isinstance(gmv, list):
            self.gmv_ranges = gmv
        else:
            self.gmv_ranges = all_gmv_ranges[1:]
        
        # 解析销售数量范围（修正逻辑：根据最小值选择所有符合的范围）
        sales = strategy.get("min_sales", "10+")
        
        if isinstance(sales, str):
            # 提取数字部分
            import re
            match = re.search(r'(\d+)', sales)
            if match:
                min_sales_val = int(match.group(1))
                self.sales_ranges = []
                
                # 根据最小值选择符合的范围
                if min_sales_val <= 0:
                    self.sales_ranges = ["0-10", "10-100", "100-1K", "1K+"]
                elif min_sales_val <= 10:
                    self.sales_ranges = ["10-100", "100-1K", "1K+"]
                elif min_sales_val <= 100:
                    self.sales_ranges = ["100-1K", "1K+"]
                elif min_sales_val <= 1000:
                    self.sales_ranges = ["1K+"]
                else:
                    self.sales_ranges = []  # 如果最小值太高，没有符合的范围
            else:
                # 如果无法解析，使用默认
                self.sales_ranges = ["10-100", "100-1K", "1K+"]
        elif isinstance(sales, (int, float)):
            min_sales_val = int(sales)
            self.sales_ranges = []
        ...
        # 解析参与率
        engagement = strategy.get("min_engagement_rate", "4%")
        if isinstance(engagement, str):
            self.min_engagement_rate = float(engagement.replace("%", ""))
        else:
    ...
    def search_and_filter(self, page, search_keyword: str = None) -> bool:
        ...
        try:
            ...
            if self.product_categories:
                ...
                # 循环处理每个类别
                for idx, category in enumerate(self.product_categories, start=1):
                    category_clean = category.strip()
                    value = category_mapping.get(category_clean)
            ...
            # 2.2) Follower count >= min_fans（使用参数）
            if self.min_fans and self.min_fans > 0:
                self.logger.info(f"步骤2.2：设置 Follower count >= {self.min_fans}")
                ensure_panel_active("Creators", creators_panel_selectors)
                click_first([
                    'button.arco-btn-outline:has-text("Follower count")',  # 通过outline按钮类型
                    'button:has(div:text-is("Follower count"))',  # 精确匹配文本
                    'button.gVxdSB:has-text("Follower")',  # 通过特定class
                    'button:has(svg.alliance-icon-ArrowDown):has-text("Follower")'  # 通过下拉箭头图标
                ], "Follower count")

                if fill_first([
                    'xpath=//*[@id="followerSize"]//input[1]',
                    '#followerSize input',
                    'div#followerSize input[type="text"]',
                    'xpath=//div[contains(@id,"follower") or contains(@id,"Follower")]//input[1]',
                    'xpath=(//div[@role="dialog"]//input)[1]'
                ], str(self.min_fans), "Follower 下限"):
                    applied["creators"]["follower_min"] = self.min_fans
            else:
                self.logger.info("跳过 Follower count 筛选（min_fans <= 0）")
            self.click_blank_area(page)

            # 2.3) Content type（使用参数）
            ...
            self.logger.info(f"Creators：Product category = {', '.join(applied['creators']['product_category']) or '无'}; "
                        f"Follower >= {applied['creators']['follower_min']}; "
    ...
    def _open_detail_and_get_page(self, page, creator_name: str):
        ...
        new_page = None
        try:
            new_page = new_page_ctx.value  # 如果真的开了新标签页，这里能拿到
        except PlaywrightTimeoutError:
            pass  # 没开新标签页
        ...
        try:
            ...
            try:
                ...
                if unavailable_locator.count() > 0:
                    ...
                    close_modal_xpath = '//span[@class="arco-icon-hover arco-modal-close-icon"]'
                    try:
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                if attempt < max_retries:
                    ...
                    self.logger.error(f"达人 {creator_name} 处理最终失败，跳过")
                    return None
    ...
    def run(self, search_keyword: str = None, max_creators: Optional[int] = None) -> bool:
        ...
        with sync_playwright() as p:
            try:
                ...
                self.target_new_count = 50   # 目标新增50个达人
                max_creators_to_load = 500  # 先加载的达人数量上限（可配置）
                ...
                for idx, creator_name in enumerate(all_loaded_creators):
                    ...
                    if creator_data:
                        ...
                        elif creator_data is None and creator_name in self.existing_data:
                            self.logger.info(f"达人 {creator_name} 之前已处理，不计入新增")
                            continue
                    else:
                ...
                self.logger.info(f"  - 跳过已处理: {len([c for c in all_loaded_creators[:idx+1] if c in self.existing_data])} 个")
                self.logger.info(f"  - 实际扫描: {idx + 1}/{len(all_loaded_creators)} 个")
                self.logger.info("=" * 50)
                return True
