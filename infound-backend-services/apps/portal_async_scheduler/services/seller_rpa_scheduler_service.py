...
from apps.portal_async_scheduler.services.seller_rpa_sample_monitor_plan_service import (
...
class SellerRpaSchedulerService:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._running_recovery_interval_seconds = max(
            int(self.scheduler_settings.running_recovery_interval_seconds or 60),
            10,
        )
    ...
    async def start(self) -> None:
        ...
        self._tasks = []
        if self.scheduler_settings.delayed_dispatch_run_in_process:
            self._tasks.append(asyncio.create_task(self._run_delayed_dispatch_loop()))
        if self.scheduler_settings.sample_monitor_daily_run_in_process:
            self._tasks.append(asyncio.create_task(self._run_sample_monitor_daily_plan_loop()))
        self.logger.info(
            "Seller RPA scheduler 已启动",
            delayed_poll_interval_seconds=self.scheduler_settings.delayed_poll_interval_seconds,
            delayed_dispatch_run_in_process=self.scheduler_settings.delayed_dispatch_run_in_process,
            sample_monitor_daily_run_in_process=self.scheduler_settings.sample_monitor_daily_run_in_process,
    ...
    async def recover_invalid_running_task_plans(self) -> int:
        return await self._recover_invalid_running_task_plans()

    async def dispatch_due_delayed_task_plans_once(self) -> bool:
        return await self._dispatch_due_task_plans_once()

    async def ensure_sample_monitor_daily_plans(self) -> int:
