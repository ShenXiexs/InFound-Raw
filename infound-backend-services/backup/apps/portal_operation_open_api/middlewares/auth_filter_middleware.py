...
from apps.portal_operation_open_api.services.security import decode_access_token, is_token_valid_in_redis, \
...
class AuthFilterMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint):
        # 排除不需要验证的路径（如登录、文档等）
        exclude_paths = ["/auth/login", "/", "/docs", "/redoc", "/openapi.json"]
        if request.url.path in exclude_paths:
            return await call_next(request)

        # 1. 获取 Header 中的 AccessToken
        token = request.headers.get(app_constants.OPERATION_ACCESS_TOKEN_HEADER)
        if not token:
            return JSONResponse(
                status_code=401,
