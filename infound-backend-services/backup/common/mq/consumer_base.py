    ...
    async def process_message(self, message: AbstractIncomingMessage) -> None:
        ...
        async with message.process():
            ...
            try:
                ...
                self.logger.error("Invalid message format", message_id=message_id, error=str(exc))
                await message.reject(requeue=False)
            except MessageProcessingError as exc:
                self.logger.error("Message processing failed", message_id=message_id, error=str(exc), exc_info=True)
    ...
    async def start(self) -> None:
        ...
        while True:
            try:
                ...
                self.logger.error("Consumer error, restarting...", error=str(exc), exc_info=True)
