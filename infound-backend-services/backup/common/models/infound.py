...
class SampleCrawlLogs(Base):
    ...
    chatbot_shipped_sent_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DATETIME(fsp=3), comment='样品发货提醒发送时间（UTC）'
    )
    chatbot_content_pending_sent_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DATETIME(fsp=3), comment='内容待发布提醒发送时间（UTC）'
    )
    no_content_next_run_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        DATETIME(fsp=3), comment='无内容发布提醒下次发送时间（UTC）'
    )
    no_content_run_count: Mapped[Optional[int]] = mapped_column(
        INTEGER, comment='无内容发布提醒已发送次数'
    )
    no_content_active: Mapped[Optional[Any]] = mapped_column(
        BIT(1), comment='无内容发布提醒是否激活'
    )
