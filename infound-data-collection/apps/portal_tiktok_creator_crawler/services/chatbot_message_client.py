...
class ChatbotMessageClient:
    """HTTP client used to publish chatbot messages via the inner API."""
    ...
    async def submit(self, tasks: List[Dict[str, Any]]) -> Dict[str, Any]:
        if not tasks:
            raise MessageProcessingError("No chatbot tasks to publish")
