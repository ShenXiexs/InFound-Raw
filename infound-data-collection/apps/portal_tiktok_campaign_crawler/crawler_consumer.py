...
from .services.campaign_crawler_service import CampaignCrawlerService


class CrawlerConsumer(ConsumerBase):
    """Campaign crawler consumer that validates login per incoming task."""
    ...
    def __init__(self) -> None:
        ...
        self.logger = get_logger().bind(consumer="portal_tiktok_campaign_crawler")
    ...
    async def _run_single_task(self, message_id: str, body: Dict[str, Any]) -> None:
        service = CampaignCrawlerService()
        try:
            await service.run_from_payload(body)
            self.logger.info(
                "Campaign login completed",
                message_id=message_id,
            )
        except Exception as exc:
            self.logger.error(
                "Campaign crawler failed",
                message_id=message_id,
                error=str(exc),
                exc_info=True,
            )
            ...
            try:
                ...
                self.logger.warning("Failed to close campaign crawler", exc_info=True)
