...
class CampaignIngestionClient:
    """HTTP client used to push campaign/product rows to the backend inner API."""
    ...
    def __init__(
        ...
        self.campaign_url = urljoin(base_url.rstrip("/") + "/", campaign_path.lstrip("/"))
        self.product_url = urljoin(base_url.rstrip("/") + "/", product_path.lstrip("/"))
        self.header_name = header_name
        self.token = token
        self.timeout = timeout
        self._client: Optional[httpx.AsyncClient] = None

    async def submit_campaigns(
        self,
        *,
        source: str,
        operator_id: str,
    ...
    async def _submit(
        ...
        *,
        ...
        logger.info(
            "Campaign ingestion submitted",
            url=url,
            rows=len(rows),
            ingested=result.get("data", {}).get(log_key),
        )
