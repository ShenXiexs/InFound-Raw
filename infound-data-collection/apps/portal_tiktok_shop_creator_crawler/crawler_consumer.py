...
from .services.shop_outreach_crawler_service import ShopOutreachCrawlerService


class CrawlerConsumer(ConsumerBase):
    """Shop-end outreach crawler consumer."""
    ...
    def __init__(self) -> None:
        ...
        self.logger = get_logger().bind(consumer="portal_tiktok_shop_creator_crawler")
    ...
    async def _run_single_task(self, message_id: str, body: Dict[str, Any]) -> None:
        service = ShopOutreachCrawlerService()
        try:
            await service.run_from_payload(body)
            self.logger.info("Shop outreach crawl completed", message_id=message_id)
        except Exception as exc:
            self.logger.error(
                "Shop outreach crawler failed",
            ...
            try:
                await asyncio.shield(service.close())
            except Exception:
                self.logger.warning("Failed to close shop outreach crawler", exc_info=True)
