...
from .services.shop_chatbot_dispatcher_service import (
    ShopChatbotDispatchTask,
    ShopChatbotDispatcherService,
)


class CrawlerConsumer(ConsumerBase):
    """Shop chatbot dispatcher consumer (sender only; supports batch tasks)."""
    ...
    def __init__(self, *, rabbitmq_config: Optional[dict] = None) -> None:
        ...
        self.logger = get_logger().bind(consumer="shop_chatbot")
        self._idle_event: Optional[asyncio.Event] = None
        self.dispatcher = ShopChatbotDispatcherService()

    async def start(self) -> None:
        if not getattr(self.settings, "CHATBOT_ENABLED", True):
            self.logger.warning("Shop chatbot disabled via config; idling")
    ...
    async def process_message_body(self, message_id: str, body: object) -> None:
        ...
        for idx, raw in enumerate(items, start=1):
            ...
            try:
                task = ShopChatbotDispatchTask.from_payload(payload)
                await self.dispatcher.dispatch(task)
                success_count += 1
                self.logger.info(
                    "Shop chat task dispatched",
                    batch_task_id=batch_task_id,
                    task_index=idx,
                ...
                error_count += 1
                self.logger.error(
                    "Shop chat dispatch failed; resetting browser and dropping batch",
                    batch_task_id=batch_task_id,
                    task_index=idx,
                ...
                raise MessageProcessingError("Playwright session reset; drop current message") from exc
                ...
                self.logger.error(
                    "Shop chat task dispatch failed",
                    batch_task_id=batch_task_id,
                    task_index=idx,
                    task_id=payload.get("taskId"),
                    message_count=len(payload.get("messages") or []),
        ...
        *,
        ...
        try:
            await dlx_exchange.publish(
                ...
                routing_key=f"{self.rabbitmq_conn.routing_key}.dead",
            )
        except Exception as exc:
            self.logger.error("Failed to publish to DLQ", error=str(exc))
