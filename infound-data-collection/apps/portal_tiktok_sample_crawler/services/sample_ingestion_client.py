...
class SampleIngestionClient:
    """HTTP client used to push normalized rows to the backend inner API."""
    ...
    def __init__(
        ...
        self.url = urljoin(base_url.rstrip("/") + "/", sample_path.lstrip("/"))
    ...
    async def submit(
        ...
        *,
        ...
        camel_options = _camelize_shallow(options or {})
        camel_rows = [_camelize_shallow(row) for row in rows]
        payload = {
            "source": source,
            "operatorId": operator_id,
            "options": camel_options,
            "rows": camel_rows,
        }
        headers = {
