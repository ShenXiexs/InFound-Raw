...
3. `apps/portal_tiktok_sample_crawler_html/configs/base.yaml` 中新增 `INNER_API` 配置，可通过环境变量覆盖：
   ...
   INNER_API_AUTH:
     ...
     VALID_TOKENS: ["844a22c4c6d47231bb4fc89c3e8a64ca5b09ec5de8f6d4bf65dacd7d"]
   ```
...
- 当前内置账号：
  - `MX1`（region MX）：`tiktokshopinfoundtest@gmail.com` / Gmail App 密码 `cfhlfedjqhfbbbhb`（含登录密码）
  - `MX2`（region MX）：`naileditz88@gmail.com` / Gmail App 密码 `ceezvbpmacwxwdck`
  - `MX3`（region MX）：`naileditz888@gmail.com` / Gmail App 密码 `oirkmvrnhfcyoqkq`
  - `MX4`（region MX，已禁用，需要luna启动程式码）：`luna.lu.shein@gmail.com`（无密码占位）
  - `FR1`（region FR）：`tiktokinfoundfrance@gmail.com` / Gmail App 密码 `gnseoovwafbstjbt`
