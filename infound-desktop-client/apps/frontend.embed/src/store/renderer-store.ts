import type { AppInfo, AppSetting, AppState, CurrentUserInfo } from '@infound/desktop-base'
import { defineStore } from 'pinia'
import { debounce, set } from 'radash'
import { IPC_CHANNELS } from '../common/ipc-type.ts'
  ...
  private useGlobalSharedStore = defineStore('globalShared', {
    ...
    actions: {
      ...
      async initSync() {
        ...
        window.logger.debug(`Renderer state synced: ${JSON.stringify(result.data)}`)
        if (result.success) {
