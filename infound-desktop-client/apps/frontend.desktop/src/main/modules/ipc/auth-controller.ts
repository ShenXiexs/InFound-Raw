...
export class AuthController {
  @IPCHandle(IPCGateway.APP, IPC_CHANNELS.APP_AUTH_LOGIN, IPCType.INVOKE)
  ...
  @IPCHandle(IPCGateway.APP, IPC_CHANNELS.APP_AUTH_LOGOUT, IPCType.INVOKE)
  ...
  @IPCHandle(IPCGateway.APP, IPC_CHANNELS.APP_AUTH_GET_CURRENT_USER, IPCType.INVOKE)
  ...
  @IPCHandle(IPCGateway.APP, IPC_CHANNELS.APP_AUTH_CHECK_TOKEN, IPCType.INVOKE)
  async checkToken(): Promise<{ success: boolean; data?: Record<string, any>; error?: string }> {
    try {
      const result = await checkTokenAsync()
      if (result?.code === 200) {
        return { success: true, data: result.data || {} }
      }
      return { success: false, error: result?.msg || '校验 token 失败' }
    } catch (error: any) {
